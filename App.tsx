...
import { STATIC_SETTINGS } from './data/staticSettings';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
      ...
      // LOAD SETTINGS (Merge Static + Local)
      // Priority: STATIC > LOCAL > DEFAULT
      // Actually, if we want Static to be the "Live" control, it should override Local.
      // But Admin Dashboard updates Local. 
      // Strategy: Load Local, then Override with Static IF Static values are present/changed?
      // Simpler: Merge Static on top of everything. The Admin Dashboard edits will only affect LocalStorage of Admin.
      // To update for everyone, Admin must edit staticSettings.ts in GitHub.
      
      const storedSettings = localStorage.getItem('nst_system_settings');
      let mergedSettings = { ...state.settings };
      ...
      // OVERRIDE WITH STATIC SETTINGS
      mergedSettings = { ...mergedSettings, ...STATIC_SETTINGS };
